    ...
    public static void main( String[] args) throws UnknownHostException {
        ...
        Undertow server = Undertow.builder()
                ...
                .setHandler(new IPAddressAccessControlHandler(new ResourceHandler(new FileResourceManager(new File(args[1])), exchange -> {
                    ...
                    map.put("tittle",exchange.getRequestURI().substring(0,exchange.getRequestURI().length()-1));
                    exchange.getResponseSender().send(new String(Freemarker.process(map,"video.html")));
